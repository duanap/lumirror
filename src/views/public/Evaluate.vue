...
const terminalCodes = new Set(['SESSION_EXPIRED','ACTIVITY_UNAVAILABLE','INVITE_DISABLED','VERIFY_DISABLED','EXPIRED','NOT_FOUND'])
...
function resetScores() { Object.keys(scores).forEach((key) => delete scores[key]) }
...
function isTerminal(error:unknown) {
  return error instanceof ApiError && (error.status === 401 || terminalCodes.has(error.code))
}
...
async function loadTask() {
  ...
  try {
    ...
    if (error instanceof ApiError && error.code === 'COMPLETED') {
      await router.replace('/success')
      return
    }
    if (isTerminal(error)) {
      await finishSession(detail(error,'评价会话已失效'))
      return
    }
    loadError.value = detail(error,'暂时无法获取评价任务，请检查网络后重试')
...
async function reconcileSubmission(submittedTaskId:string) {
  try {
    const current = unwrap(await api.get<EvaluationTask>('/public/current-task'))
    if (current.id !== submittedTaskId) {
      task.value = current
      resetScores()
      ElMessage.success('提交状态已确认，已进入下一项评价')
      return true
    }
  } catch (error) {
    if (error instanceof ApiError && error.code === 'COMPLETED') {
      ElMessage.success('提交状态已确认')
      await router.replace('/success')
      return true
    }
    if (isTerminal(error)) {
      await finishSession(detail(error,'评价会话已结束'))
      return true
    }
  }
  return false
...
async function submit() {
  ...
  if (timedExpired.value) {
    await finishSession('评价不存在或已结束')
    return
  }
  try {
    await ElMessageBox.confirm(`本次综合评分为 ${total.value} 分，提交后不可修改。`,'确认提交',{confirmButtonText:'确认提交',cancelButtonText:'再检查一下',type:'warning'})
  } catch { return }
  const submittedTaskId = task.value.id
  submitting.value = true
  try {
    const result = unwrap(await api.post<{remaining:number;completed:boolean}>('/public/submit-score',{taskId:submittedTaskId,scores}))
    ElMessage.success('提交成功')
    if (result.completed) await router.replace('/success')
    else await loadTask()
  } catch (error) {
    if (error instanceof ApiError && (error.code === 'TASK_ALREADY_SUBMITTED' || error.code === 'NETWORK_ERROR' || error.code === 'TIMEOUT' || error.status >= 500)) {
      if (await reconcileSubmission(submittedTaskId)) return
      ElMessage.warning('尚未能确认提交状态，已保留当前评分，请检查网络后再重试')
      return
    }
    if (isTerminal(error)) {
      await finishSession(detail(error,'评价会话已结束'))
    ...
    ElMessage.error(detail(error,'提交失败'))
...
<template>
  <PublicShell compact>
    ...
    <div v-else-if="!task" class="score-card loading-card">
      <p>{{ loadError || '暂时无法获取评价任务' }}</p>
      <el-button type="primary" @click="loadTask">重新加载</el-button>
    </div>
    <article v-else class="score-card">
