  ...
  <AdminPage title="综合评分计算规则" description="每个评价活动独立保存规则，切换活动后自动刷新">
    <div v-loading="loading">
      ...
      <section class="panel rules">
        <div class="rules-head"><div><h3>评分维度与计入方式</h3><small>加分比例之和减去减分比例之和，应等于 100%</small></div><el-button type="primary" plain :icon="Plus" @click="addRule">新增维度</el-button></div>
        <el-table :data="rules">
          ...
          <el-table-column label="最低分" min-width="140"><template #default="{ row }"><el-input-number v-model="row.min" :min="0" :max="98"/></template></el-table-column>
          ...
          <el-table-column label="计入方向" min-width="140"><template #default="{ row }"><el-select v-model="row.operation"><el-option label="增加" value="add"/><el-option label="减少" value="subtract"/></el-select></template></el-table-column>
          ...
          <el-table-column label="操作" width="80" fixed="right"><template #default="{ $index }"><el-button link type="danger" :icon="Delete" aria-label="删除评分维度" @click="removeRule($index)"/></template></el-table-column>
        </el-table>
...
<style scoped>
.panel{margin-top:18px;padding:20px 22px}h3{margin:0 0 16px;font-size:17px}.rules-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:16px}.rules-head h3{margin-bottom:4px}.rules-head small{color:var(--muted)}.scope-grid{display:grid;grid-template-columns:1fr 1.2fr 150px;gap:20px;align-items:end}.scope-grid label{display:grid;gap:8px}.scope-grid span{font-size:14px}.rules .el-input-number{width:116px}.percent{margin-left:7px;color:var(--muted)}.validation{display:flex;align-items:center;gap:8px;margin-top:12px;color:var(--success)}.validation svg{width:18px}.validation.invalid{color:var(--danger)}.preview-settings{display:grid;grid-template-columns:1.3fr 1fr;gap:32px}.preview-box{padding:17px;border:1px solid var(--brand-soft);border-radius:10px;background:var(--surface-soft)}.preview-title{display:flex;align-items:center;gap:10px;margin-bottom:13px;font-weight:700}.preview-title svg{width:20px;color:var(--brand)}.preview-box strong{display:block;margin-left:30px;color:var(--brand)}.preview-box small{display:block;margin:10px 0 0 30px;color:var(--muted)}.rounding .el-radio-group{display:grid;gap:14px}.savebar{display:flex;align-items:center;justify-content:flex-end;gap:12px;margin-top:20px}.warning{display:flex;align-items:center;gap:8px;margin-right:auto;color:var(--warning);font-size:14px}.warning svg{width:18px;color:var(--warning)}
