...
const form = reactive<Employee>({id:'',name:'',gender:'male',departmentId:'',teamId:'',position:'',status:'active'})
...
function add() {
  ...
  Object.assign(form,{id:'',name:'',gender:'male',departmentId:firstTeam?.departmentId||'',teamId:firstTeam?.id||'',position:'',status:'active'})
  dialog.value = true
}
function edit(row:Employee) { Object.assign(form,row); dialog.value = true }
async function save() {
  ...
  <AdminPage title="成员管理" description="按部门和团队筛选成员，维护参与评价的人员、岗位与状态">
    ...
    <div class="panel table-wrap">
      <el-table v-loading="loading" :data="rows" row-key="id" @selection-change="onSelectionChange">
        ...
        <el-table-column label="成员" min-width="175"><template #default="{row}"><div class="person"><img :src="row.avatar||`/assets/avatar/default-${row.gender==='female'?'female':'male'}.svg`"/><div><b>{{row.name}}</b><small>{{row.position || '未设置岗位'}}</small></div></div></template></el-table-column>
        <el-table-column v-if="columns.visible('gender')" label="性别" width="80"><template #default="{row}"><el-tag size="small" :type="row.gender==='female'?'danger':row.gender==='male'?'primary':'info'">{{row.gender==='female'?'女':row.gender==='male'?'男':'未知'}}</el-tag></template></el-table-column>
...
<style scoped>
.filter-panel{display:grid;grid-template-columns:minmax(200px,1.4fr) repeat(3,minmax(140px,1fr)) auto auto auto;gap:10px;margin:20px 0 16px;padding:14px}.batch-bar{display:flex;align-items:center;gap:10px;margin:0 0 16px;padding:10px 14px}.batch-bar span{margin-right:auto;color:var(--muted)}.batch-bar b{color:var(--brand)}.person{display:flex;align-items:center;gap:10px}.person img{width:42px;height:42px;border-radius:50%;background:var(--surface-tint)}.person small{display:block;margin-top:4px;color:var(--subtle)}.form-grid{display:grid;grid-template-columns:1fr 1fr;gap:0 18px}@media(max-width:1180px){.filter-panel{grid-template-columns:repeat(2,1fr)}}@media(max-width:600px){.filter-panel,.form-grid{grid-template-columns:1fr}.filter-panel{padding:12px}.batch-bar{align-items:stretch;flex-direction:column}}
</style>
