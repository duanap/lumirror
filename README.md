...
- 每次评价聚焦一位成员，展示头像、姓名、性别、团队和岗位；提交后自动进入下一位待评价成员，全部完成后进入成功页。
- 默认评分维度为“工作能力、工作态度、协作能力”。输入框即时校验分数范围，显示完成状态，并实时计算综合总分。
- 默认活动在总分下直接展示：`综合总分=(工作能力 + 工作态度 + 协作能力) / 3`。
...
- 可按评价周期创建评价活动，并同步生成邀请链接、邀请码和评价任务；支持邀请码下载、链接复制、时效链接、活动停用与删除。
...
后台评分规则已经支持自定义维度及增加/减少计入，但公开评价页的公式提示目前固定为默认三项平均分。因此，当活动改用自定义规则时，服务端计算仍正确，页面提示却可能与实际计算不一致。

建议后续将公开评价页的固定文案改为根据当前活动已启用规则动态生成：默认三项等权时继续显示现有公式；存在自定义维度、权重或减少项时，展示与后台 `ruleFormula` 相同的动态说明。这样既保留默认活动的直观表达，也保证自定义活动的说明和实际计算一致。
